...
st.markdown('# <div style="text-align:center"> [Previsão de renda](https://github.com/Thomaz-bp/previsao-renda) </div>',
            unsafe_allow_html=True)
...
buffer = io.StringIO()
dfrenda.info(buf=buffer)
st.text(buffer.getvalue())
st.dataframe(dfrenda)


st.table(dfrenda.nunique()
              .to_frame()
              .reset_index()
...
st.write(dfrenda.describe().transpose())
...
cmap = sns.diverging_palette(h_neg=100,
                             ...
                             sep=1,
                             center='light')
ax = sns.clustermap(data=dfrenda.corr(numeric_only=True),
                    figsize=(10, 10),
                    center=0,
                    cmap=cmap)
...
with st.expander("Análise das variáveis qualitativas ao longo do tempo", expanded=True):
    dfrenda['data_ref'] = pd.to_datetime(arg=dfrenda['data_ref'])
    qualitativas = dfrenda.select_dtypes(include=['object', 'boolean']).columns
    plt.rc('figure', figsize=(16, 4))
    for col in qualitativas:
        fig, axes = plt.subplots(nrows=1, ncols=2)
        fig.subplots_adjust(wspace=.6)
        tick_labels = dfrenda['data_ref'].map(
            lambda x: x.strftime('%b/%Y')).unique()
        # barras empilhadas:
        renda_crosstab = pd.crosstab(index=dfrenda['data_ref'],
                                     columns=dfrenda[col],
                                     normalize='index')
...
st.table(pd.DataFrame(index=dfrenda.nunique().index,
                      data={'tipos_dados': dfrenda.dtypes,
                            'qtd_valores': dfrenda.notna().sum(),
                            'qtd_categorias': dfrenda.nunique().values}))
...
with st.expander("Conversão das variáveis categóricas em variáveis numéricas (dummies)", expanded=True):
    ...
    buffer = io.StringIO()
    dfrenda_dummies.info(buf=buffer)
    st.text(buffer.getvalue())

    st.table((dfrenda_dummies.corr(numeric_only = True)['renda']
...
dfrenda['renda_predict'] = np.round(reg_tree.predict(X), 2)
st.dataframe(dfrenda[['renda', 'renda_predict']])
